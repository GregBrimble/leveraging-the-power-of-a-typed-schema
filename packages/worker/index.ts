...
const handleGraphQLRequest = async ({ request }: FetchEvent) => {
  const response = (request.method === 'OPTIONS') ? new Response(null, { status: 204 }) : await graphQL(request);
...
const handleAssetRequest = async (event: FetchEvent) => {
  try {
